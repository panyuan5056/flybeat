...
func init() {
	for tag, regex := range Patterns {
		Rows = append(Rows, map[string]string{"label": tag, "value": regex})
	}

	matches, _ := filepath.Glob("pkg/patterns/*")
	for _, v := range matches {
		if !strings.HasSuffix(v, ".go") {
			f, err := os.OpenFile("./"+v, os.O_RDONLY, 0600)
			defer f.Close()
			if err != nil {
				...
				for _, row := range items {
					if !strings.HasPrefix(row, "#") && len(row) > 3 {
						cols := strings.SplitN(row, " ", 2)
						if len(cols) == 2 {
							tmp = append(tmp, map[string]string{"label": cols[0], "value": cols[1]})
						}
					}
				}
				Info[strings.Replace(v, "pkg/patterns/", "", 1)] = tmp
			}
		}
	}
}
